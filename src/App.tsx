...
function useNavigation() {
  ...
  const nav = useCallback((newView: string) => {
    scrollPositions.current[view] = window.scrollY;
    window.history.pushState({ view: newView }, "", "");
    setViewState(newView);
    const saved = scrollPositions.current[newView] || 0;
  ...
  useEffect(() => {
    const onPop = (e: PopStateEvent) => {
      ...
      scrollPositions.current[view] = window.scrollY;
      setViewState(v);
      const saved = scrollPositions.current[v] || 0;
      setTimeout(() => window.scrollTo(0, saved), 50);
    };
...
// ─── TESTIMONIALS ─────────────────────────────────────────────────────────────
const TESTIMONIALS = [
  {name:"Sarah M.", location:"Lincoln, NE → Dallas, TX", pet:"Golden Retriever", text:"Found a driver going exactly our route within 2 days. Buddy arrived happy and safe. Saved us over $800 compared to professional couriers!", avatar:"🐕", rating:5},
  {name:"Anna K.", location:"Berlin → Munich", pet:"Persian Cat", text:"Luna gets anxious in cargo. With PetAlong she rode in the cabin the whole way. The driver was amazing, sent updates every hour.", avatar:"🐈", rating:5},
  {name:"Carlos M.", location:"Phoenix, AZ → Houston, TX", pet:"Parrot", text:"Didn't think anyone would take a parrot but found a bird lover driver in 3 days. Raven loved the trip apparently!", avatar:"🦜", rating:5},
  {name:"Mike D.", location:"Chicago, IL → Miami, FL", pet:"Two Rabbits", text:"As a driver I've taken 6 pets so far. Great extra income on trips I was already making. The platform is simple and the owners are always so grateful.", avatar:"🐇", rating:5},
  {name:"Jennifer L.", location:"Denver, CO → Los Angeles", pet:"Labrador", text:"Max is 80lbs so flying wasn't an option. PetAlong matched us with a SUV driver in 4 days. Honestly the best pet transport experience we've had.", avatar:"🐕", rating:5},
];

export default function App() {
  const { view, nav } = useNavigation();
  ...
  const calcDistance = async (from: string, to: string) => {
    if (!from || !to) return;
    setDistanceInfo("Calculating...");
    try {
      const url = `https://maps.googleapis.com/maps/api/distancematrix/json?origins=${encodeURIComponent(from)}&destinations=${encodeURIComponent(to)}&units=imperial&key=AIzaSyD-placeholder`;
      setDistanceInfo(null); // Google Maps API requires billing — show helpful message instead
      const fromEnc = encodeURIComponent(from);
      const toEnc = encodeURIComponent(to);
      window.open(`https://www.google.com/maps/dir/${fromEnc}/${toEnc}`, "_blank");
    } catch { setDistanceInfo(null); }
  };

  const postOwnerListing = async () => {
    if (!oF.from || !oF.to) { alert("Please enter pickup and delivery locations"); return; }
    setPosting(true);
    try {
      let photoUrl = null;
      if (petPhotoFile) {
        photoUrl = await uploadPhoto(petPhotoFile);
      }
      await sbFetch("/listings", {
        method: "POST",
        body: JSON.stringify({
          type: "owner",
          from_location: oF.from, to_location: oF.to,
          country: selectedCountry,
          animal_name: oF.name, pet_type: oF.petType,
          weight: Number(oF.weight) || 0,
          date_from: oF.d1, date_to: oF.d2,
          avatar: PET_ICONS[oF.petType] || "🐾",
          open_chat: oF.openChat, open_calls: oF.openCalls, open_sms: oF.openSMS,
          phone: oF.ph, notes: oF.notes,
          pet_photo: photoUrl,
          user_name: loggedIn ? "Vova78" : "Guest",
          fb_connected: loggedIn, phone_verified: false,
          member_since: "2026", rating: 5.0, reviews: 0,
        }),
      });
      nav("posted");
    } catch (e) { alert("Error posting. Please try again."); }
    setPosting(false);
  };

  const postDriverListing = async () => {
    if (!dF.from || !dF.to) { alert("Please enter your route"); return; }
    setPosting(true);
    try {
      await sbFetch("/listings", {
        method: "POST",
        body: JSON.stringify({
          type: "driver",
          from_location: dF.from, to_location: dF.to,
          country: selectedCountry,
          weight: Number(dF.cap) || 0,
          date_from: dF.date, avatar: "🚐",
          open_chat: dF.openChat, open_calls: dF.openCalls, open_sms: dF.openSMS,
          phone: dF.ph, notes: dF.notes,
          radius_from: dF.rA, radius_to: dF.rB,
          user_name: loggedIn ? "Vova78" : "Guest",
          fb_connected: loggedIn, phone_verified: false,
          member_since: "2026", rating: 5.0, reviews: 0,
        }),
  ...
  const NavBar = ({showPost=false}) => (
    <nav style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 20px",borderBottom:`1px solid ${C.border2}`,position:"sticky",top:0,background:"rgba(13,17,23,0.97)",backdropFilter:"blur(14px)",zIndex:200}}>
      <div onClick={()=>nav("home")} style={{fontWeight:900,fontSize:20,cursor:"pointer",letterSpacing:-0.5,color:C.white}} translate="no">
        Pet<span style={{color:C.orange}}>Along</span>
      </div>
      <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap" as const}}>
        {showPost && <button onClick={()=>nav("owner")} style={{background:C.orange,border:"none",borderRadius:100,padding:"7px 14px",color:C.bg,fontWeight:700,fontSize:12,cursor:"pointer"}}>+ Post</button>}
        {loggedIn
          ? <div onClick={()=>nav("profile-me")} style={{width:32,height:32,borderRadius:"50%",background:`linear-gradient(135deg,${C.orange},${C.red})`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:13,fontWeight:700,color:C.bg}}>V</div>
          : <button onClick={()=>setShowLogin(true)} style={{background:"transparent",border:`1px solid ${C.border}`,borderRadius:100,padding:"6px 12px",color:C.muted,fontSize:12,cursor:"pointer"}}>Sign in</button>
        }
        <div id="google_translate_element" style={{fontSize:11}}></div>
      </div>
    </nav>
  );
  ...
  const CountryPicker = () => (
    <div style={{marginBottom:16,position:"relative"}}>
      ...
      <div onClick={()=>setShowCountries(!showCountries)} style={{...INP,display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",userSelect:"none" as const}}>
        <span style={{color:"#1a1a1a",fontWeight:500}}>{selectedCountry}</span>
        <span style={{color:"#555",fontSize:18,lineHeight:1}}>⌄</span>
      </div>
      {showCountries && (
        <div style={{position:"absolute",top:"100%",left:0,right:0,background:"#fff",border:`2px solid ${C.orange}`,borderRadius:10,zIndex:50,maxHeight:220,overflowY:"auto" as const,boxShadow:"0 8px 30px rgba(0,0,0,.3)"}}>
          <input autoFocus placeholder="Search country..." value={countrySearch} onChange={e=>setCountrySearch(e.target.value)} style={{...INP,borderRadius:"8px 8px 0 0",borderBottom:`1px solid #eee`,borderLeft:"none",borderRight:"none",borderTop:"none"}}/>
  ...
  const SubmitBtn = ({label,onClick,disabled=false}:{label:string,onClick:()=>void,disabled?:boolean}) => (
    <button onClick={onClick} disabled={disabled} style={{width:"100%",background:disabled?"#444":C.orange,border:"none",borderRadius:14,padding:17,color:C.bg,fontWeight:700,fontSize:16,cursor:disabled?"default":"pointer",marginTop:8}}>
      {label}
    </button>
  );
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:C.card2,border:`1px solid ${C.border}`,borderRadius:20,padding:28,width:"100%",maxWidth:360}}>
        {reportDone ? (
          <div style={{textAlign:"center" as const,padding:"20px 0"}}>
            ...
            <div style={{color:C.white,fontWeight:700,fontSize:16,marginBottom:8}}>Thank you! We'll review it shortly.</div>
      ...
      <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap" as const,marginTop:12}}>
            ...
            <button onClick={()=>{setShowReport(l);setReportReason("");}} style={{background:"transparent",border:`1px solid ${C.border}`,color:C.muted2,padding:"7px 12px",borderRadius:10,fontSize:12,cursor:"pointer"}}>🚩</button>
          </div>
        </div>
      </div>
    </div>
  ...
  if (view === "home") return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.white,fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{minHeight:"90vh",display:"flex",flexDirection:"column" as const,alignItems:"center",justifyContent:"center",padding:"60px 20px 40px",textAlign:"center" as const,position:"relative",overflow:"hidden"}}>
        ...
        <div style={{fontWeight:900,fontSize:"clamp(40px,10vw,80px)",letterSpacing:-2,lineHeight:1,marginBottom:16,color:C.white}} translate="no">
          Pet<span style={{color:C.orange}}>Along</span>
        </div>
        ...
        <div style={{display:"flex",gap:28,marginBottom:52,flexWrap:"wrap" as const,justifyContent:"center"}}>
          {[{n:"50+",l:"Countries"},{n:"Free",l:"Forever for basics"},{n:"P2P",l:"Direct connections"},{n:"0%",l:"Commission"}].map((s,i)=>(
  ...
  if (view === "feed") return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.white,fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{maxWidth:680,margin:"0 auto",padding:"28px 20px 80px"}}>
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"14px",marginBottom:20}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap" as const}}>
            <select value={filterCountry} onChange={e=>setFilterCountry(e.target.value)} style={{...INP,flex:1,minWidth:110,padding:"8px 10px",fontSize:13}}>
              <option value="all">🌍 All</option>
              {COUNTRIES.map(c=><option key={c} value={c}>{c}</option>)}
            </select>
            <select value={filterPet} onChange={e=>setFilterPet(e.target.value)} style={{...INP,flex:1,minWidth:100,padding:"8px 10px",fontSize:13}}>
        ...
        {!loading&&filtered.length===0&&(
          <div style={{textAlign:"center" as const,padding:"60px 20px",color:C.muted}}>
            ...
            <div style={{marginBottom:24,fontSize:14}}>Be the first to post!</div>
  ...
  if (view === "owner") return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.white,fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{maxWidth:540,margin:"0 auto",padding:"28px 20px 80px"}}>
        ...
        <CountryPicker/>
        <Field label="Pickup location" ph="City, State or ZIP code" val={oF.from} onChange={v=>setOF(p=>({...p,from:v}))}/>
        <Field label="Delivery location" ph="City, State or ZIP code" val={oF.to} onChange={v=>setOF(p=>({...p,to:v}))}/>
        {oF.from&&oF.to&&(
          <button onClick={()=>openMaps(oF.from,oF.to)} style={{width:"100%",background:"transparent",border:`1px solid #238636`,color:C.green,borderRadius:10,padding:"10px",fontSize:13,cursor:"pointer",fontWeight:600,marginBottom:16}}>
            🗺 View route on Google Maps: {oF.from} → {oF.to}
          </button>
        )}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <Field label="Pet name" ph="Buddy" val={oF.name} onChange={v=>setOF(p=>({...p,name:v}))}/>
          <Field label="Weight (lbs)" ph="45" type="number" val={oF.weight} onChange={v=>setOF(p=>({...p,weight:v}))}/>
        </div>
        <div style={{marginBottom:16}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap" as const}}>
            {PET_TYPES.map(p=>(
              <button key={p.en} onClick={()=>setOF(prev=>({...prev,petType:p.en}))} style={{padding:"8px 14px",borderRadius:100,border:`1px solid ${oF.petType===p.en?C.orange:C.border}`,background:oF.petType===p.en?"rgba(240,160,48,.15)":"transparent",color:oF.petType===p.en?C.orange:C.muted,fontSize:13,cursor:"pointer"}}>
                {p.icon} {p.en}
              </button>
            ))}
          </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <Field label="Ready from" ph="" type="date" val={oF.d1} onChange={v=>setOF(p=>({...p,d1:v}))}/>
          <Field label="Deliver by" ph="" type="date" val={oF.d2} onChange={v=>setOF(p=>({...p,d2:v}))}/>
        </div>
        <Field label="Notes (optional)" ph="Pet behavior, special requirements..." val={oF.notes} onChange={v=>setOF(p=>({...p,notes:v}))} multiline/>
        <Divider label="How to contact you"/>
        <Toggle label="Open for in-app chat" sub="Free messaging" checked={oF.openChat} onChange={()=>setOF(p=>({...p,openChat:!p.openChat}))}/>
        <Toggle label="Open for phone calls" checked={oF.openCalls} onChange={()=>setOF(p=>({...p,openCalls:!p.openCalls}))}/>
        <Toggle label="Open for SMS" checked={oF.openSMS} onChange={()=>setOF(p=>({...p,openSMS:!p.openSMS}))}/>
        {(oF.openCalls||oF.openSMS)&&<Field label="Your phone number" ph="+1 (555) 000-0000" type="tel" val={oF.ph} onChange={v=>setOF(p=>({...p,ph:v}))}/>}
        <SubmitBtn label={posting?"Posting...":"Post my request →"} onClick={postOwnerListing} disabled={posting}/>
  ...
  if (view === "driver") return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.white,fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{maxWidth:540,margin:"0 auto",padding:"28px 20px 80px"}}>
        ...
        <CountryPicker/>
        <Field label="Departing from" ph="City, State or ZIP code" val={dF.from} onChange={v=>setDF(p=>({...p,from:v}))}/>
        <Field label="Heading to" ph="City, State or ZIP code" val={dF.to} onChange={v=>setDF(p=>({...p,to:v}))}/>
        {dF.from&&dF.to&&(
          <button onClick={()=>openMaps(dF.from,dF.to)} style={{width:"100%",background:"transparent",border:`1px solid #238636`,color:C.green,borderRadius:10,padding:"10px",fontSize:13,cursor:"pointer",fontWeight:600,marginBottom:16}}>
            🗺 View route: {dF.from} → {dF.to}
          </button>
        )}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <Field label="Travel date" ph="" type="date" val={dF.date} onChange={v=>setDF(p=>({...p,date:v}))}/>
          <Field label="Max weight (lbs)" ph="80" type="number" val={dF.cap} onChange={v=>setDF(p=>({...p,cap:v}))}/>
        </div>
        <Divider label="Pickup detour"/>
        <Slider label="How far will you detour to pick up?" val={dF.rA} onChange={v=>setDF(p=>({...p,rA:v}))} color="orange"/>
        <Divider label="Delivery detour"/>
        <Slider label="How far will you detour to deliver?" val={dF.rB} onChange={v=>setDF(p=>({...p,rB:v}))} color="green"/>
        ...
        <Field label="Notes (optional)" ph="Vehicle type, pet experience..." val={dF.notes} onChange={v=>setDF(p=>({...p,notes:v}))} multiline/>
        <Divider label="How to contact you"/>
        <Toggle label="Open for in-app chat" sub="Free messaging" checked={dF.openChat} onChange={()=>setDF(p=>({...p,openChat:!p.openChat}))}/>
        <Toggle label="Open for phone calls" checked={dF.openCalls} onChange={()=>setDF(p=>({...p,openCalls:!p.openCalls}))}/>
        <Toggle label="Open for SMS" checked={dF.openSMS} onChange={()=>setDF(p=>({...p,openSMS:!p.openSMS}))}/>
        {(dF.openCalls||dF.openSMS)&&<Field label="Your phone number" ph="+1 (555) 000-0000" type="tel" val={dF.ph} onChange={v=>setDF(p=>({...p,ph:v}))}/>}
        <SubmitBtn label={posting?"Posting...":"Post my route →"} onClick={postDriverListing} disabled={posting}/>
        <div style={{fontSize:11,color:C.muted2,textAlign:"center" as const,marginTop:12,lineHeight:1.6}}>
          By posting you agree to our <span onClick={()=>nav("terms")} style={{color:C.orange,cursor:"pointer"}}>Terms of Service</span>
        </div>
      </div>
  ...
  const LegalPage = ({title,content}:{title:string,content:string}) => (
    <div style={{minHeight:"100vh",background:C.bg,color:C.white,fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{maxWidth:720,margin:"0 auto",padding:"48px 24px 80px"}}>
        ...
        {title==="Disclaimer"&&<div style={{background:"rgba(248,81,73,.06)",border:`1px solid rgba(248,81,73,.3)`,borderRadius:12,padding:"14px 20px",marginBottom:20,fontSize:13,color:C.red,fontWeight:600}}>⚠️ Read carefully before using PetAlong.</div>}
  ...
  if (view==="terms") return <LegalPage title="Terms of Service" content={TERMS}/>;
  if (view==="privacy") return <LegalPage title="Privacy Policy" content={PRIVACY}/>;
  if (view==="disclaimer") return <LegalPage title="Disclaimer" content={DISCLAIMER}/>;
    ...
    <div style={{minHeight:"100vh",background:C.bg,color:C.white,fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{maxWidth:720,margin:"0 auto",padding:"48px 24px 80px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column" as const,gap:14}}>
          {[{icon:"📋",t:"Post your listing",d:"Owner posts pet details with photo. Driver posts route and detour radius. Takes 2 minutes."},{icon:"🔍",t:"Get matched",d:"Your listing is shown to drivers or owners going the same way."},{icon:"💬",t:"Connect directly",d:"Chat, call or SMS. Agree on price and details between yourselves."},{icon:"🐾",t:"Pet travels safely",d:"Driver takes pet along their existing route. Everyone saves money."}].map((h,i)=>(
