    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={()=>{setShowLogin(false);setLoginStep("main");}}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.card2,border:`1px solid ${C.border}`,borderRadius:20,padding:32,width:"100%",maxWidth:380}}>
        {loginStep==="main" && <>
          ...
          <SubmitBtn label={t.sendCode} onClick={()=>setLoginStep("code")}/>
        </>}
        {loginStep==="code" && <>
          ...
          <SubmitBtn label={t.verify} onClick={()=>{setLoggedIn(true);setShowLogin(false);setLoginStep("main");}}/>
          <button onClick={()=>setLoginStep("main")} style={{background:"none",border:"none",color:C.muted,fontSize:13,cursor:"pointer",marginTop:12,width:"100%",textAlign:"center" as const}}>{t.back}</button>
        </>}
      </div>
    </div>
  );
  ...
  if(view==="profile-me") return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.white,fontFamily:"'DM Sans',system-ui,sans-serif"}}>
      ...
      <div style={{maxWidth:580,margin:"0 auto",padding:"28px 20px 80px"}}>
        ...
        <button onClick={()=>{setLoggedIn(false);nav("home");}} style={{width:"100%",background:"transparent",border:`1px solid ${C.red}`,borderRadius:12,padding:14,color:C.red,fontWeight:600,cursor:"pointer",fontSize:14}}>{t.signOut}</button>
      </div>
      <Footer/>
    </div>
  );
